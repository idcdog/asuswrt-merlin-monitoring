...
The project separates live router state, standard interface counters, and hourly traffic history into independent collection paths.
...
Official `snmp_exporter` `if_mib` metrics provide byte counters, errors, discards, interface state, and advertised speed. Authentication is stored in a separate `asus-auth.yml` that must never be committed.
